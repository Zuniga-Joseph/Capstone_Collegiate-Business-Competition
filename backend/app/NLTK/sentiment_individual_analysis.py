...
def measure_constructs_from_text(text: str, tfidf_feats: Optional[Dict] = None) -> Dict[str, Any]:
    ...
    Strictly individual analysis:
      ...
      optional: tfidf_feats: dict of TF-IDF-based lexicon features, e.g.
          {
            "si_empathy_tfidf": float,
            "si_polite_tfidf": float,
            "mr_resilience_tfidf": float,
            "mr_catastrophize_tfidf": float,
            "cg_growth_tfidf": float,
          }
    ...
    you_ct = (
        all_tokens.count("you")
        + all_tokens.count("your")
        + all_tokens.count("y’all")
        + all_tokens.count("yall")
    )
    we_ct = (
        all_tokens.count("we")
        + all_tokens.count("our")
        + all_tokens.count("us")
    )
    ...
    ack_phrases = sum(
        ...
        for s in sents_list
        if re.search(
            r"\b(i see|makes sense|got it|good point|that’s helpful|that's helpful)\b",
            s["text"].lower(),
        )
    )
    ...
    feat_MR = {
        ...
        "reappraisal_flips_per100s": (
            mr_flips / (len(sents_list) / 100.0 + 1e-9)
        ),
    ...
    feedback_cues = [
        i for i, s in enumerate(sents_list) if "feedback" in s["text"].lower()
    ]
    followup_q = sum(
        1
        for i in feedback_cues
        if i + 1 < len(sents_list)
        and QUESTION_RE.search(sents_list[i + 1]["text"])
    )
    ...
    # --- Optional: integrate TF-IDF features (if provided) ---
    tfidf_feats = tfidf_feats or {}

    feat_SI["empathy_tfidf"] = tfidf_feats.get("si_empathy_tfidf", 0.0)
    feat_SI["polite_tfidf"] = tfidf_feats.get("si_polite_tfidf", 0.0)
    feat_MR["resilience_tfidf"] = tfidf_feats.get("mr_resilience_tfidf", 0.0)
    feat_MR["catastrophize_tfidf"] = tfidf_feats.get("mr_catastrophize_tfidf", 0.0)
    feat_CG["growth_tfidf"] = tfidf_feats.get("cg_growth_tfidf", 0.0)

    # --- Score mapping ---
    def to_100(x, scale=1.5):
        return max(0, min(100, 100 * (x / (x + scale)) if x > 0 else 0))

    # You can tune these TF-IDF weights later based on data
    si_raw = (
        0.30 * feat_SI["lex_empathy_perk"]
        + 0.18 * feat_SI["lex_polite_perk"]
        + 0.20 * feat_SI["acknowledgment_rate"]
        + 0.15 * feat_SI["question_rate"]
        + 0.07 * min(feat_SI["you_we_to_i_ratio"], 3.0) / 3.0
        + 0.10 * (feat_SI["empathy_tfidf"] + feat_SI["polite_tfidf"])
    )

    mr_raw = (
        0.40 * feat_MR["resilience_lex_perk"]
        + 0.20 * feat_MR["modal_rate_perk"]
        + 0.25 * feat_MR["reappraisal_flips_per100s"]
        - 0.15 * feat_MR["catastrophize_perk"]
        + 0.10 * feat_MR["resilience_tfidf"]
        - 0.05 * feat_MR["catastrophize_tfidf"]
    )

    cg_raw = (
        0.50 * feat_CG["growth_lex_perk"]
        + 0.25 * feat_CG["feedback_accept_rate"]
        + 0.15 * feat_CG["clarify_after_feedback_rate"]
        + 0.10 * feat_CG["growth_tfidf"]
    )
    ...
    return {
        "features": {"SI": feat_SI, "MR": feat_MR, "CG": feat_CG},
        "scores": scores,
    }
